      ...
      <div className="flex flex-col gap-3">
        {LOCALES.map(({ code, label, flag }) => {
          ...
          return (
            ...
            >
              <span className={`w-10 h-10 rounded-full flex items-center justify-center text-2xl shrink-0 ${isCurrent ? 'bg-[#E6F0FE]' : 'bg-[#EFF1F5]'}`}>{flag}</span>
...
export default function SettingsPage({ currentLocale, extraSections = [] }: Props) {
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#EFF1F5] shadow-sm p-5 md:p-8">
        <div className="space-y-4">
          <h2 className="text-base font-semibold text-[#25282A]">{t('settings.support.title')}</h2>
          <a
            href="tel:+84568240240"
            className="flex items-center gap-4 px-5 py-4 rounded-2xl border-2 border-[#EFF1F5] bg-white hover:border-[#0669F7]/40 hover:bg-[#F2F4F5] transition-all"
          >
            <span className="w-10 h-10 rounded-full bg-[#EFF1F5] flex items-center justify-center shrink-0">
              <svg className="w-5 h-5 text-[#0669F7]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
            </span>
            <div className="flex-1">
              <p className="text-sm font-semibold text-[#25282A]">{t('settings.support.phone')}</p>
              <p className="text-xs text-[#98A2B2] mt-0.5">{t('settings.support.label')}</p>
            </div>

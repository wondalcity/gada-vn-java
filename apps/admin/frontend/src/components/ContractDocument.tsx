...
const CONTRACT_STRINGS = {
  ko: {
    ...
    contract_no_label: '계약번호', btn_save: '이미지로 저장', btn_saving: '이미지 생성 중...',
    filename_prefix: '계약서', lang_label: '계약서 언어',
  },
  vi: {
    ...
    contract_no_label: 'Số hợp đồng', btn_save: 'Lưu hình ảnh', btn_saving: 'Đang tạo ảnh...',
    filename_prefix: 'hop-dong', lang_label: 'Ngôn ngữ hợp đồng',
  },
  en: {
    ...
    contract_no_label: 'Contract No.', btn_save: 'Save as Image', btn_saving: 'Generating...',
    filename_prefix: 'contract', lang_label: 'Contract Language',
  },
} as const
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <tr>
      <td style={{ width: '30%', padding: '7px 10px', backgroundColor: '#F5F7FA', borderRight: '1px solid #CCCCCC', borderBottom: '1px solid #CCCCCC', fontSize: '12px', fontWeight: 600, color: '#444', whiteSpace: 'nowrap', verticalAlign: 'top' }}>
        {label}
      </td>
      <td style={{ padding: '7px 10px', borderBottom: '1px solid #CCCCCC', fontSize: '12px', color: '#222', verticalAlign: 'top' }}>
        {value}
      </td>
    </tr>
  )
...
function SectionHeader({ children }: { children: string }) {
  return (
    <tr>
      <td colSpan={2} style={{ padding: '8px 10px', backgroundColor: '#1a4fa0', color: '#fff', fontSize: '12px', fontWeight: 700, letterSpacing: '0.03em' }}>
        {children}
      </td>
    </tr>
  )
...
export function ContractLangSelector({ lang, onChange }: { lang: SupportedLang; onChange: (l: SupportedLang) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      <span className="text-xs text-[#7A7B7A] shrink-0">{s.lang_label}</span>
      <div className="flex rounded-lg border border-[#DDDDDD] overflow-hidden">
        {LANG_OPTIONS.map(opt => (
          <button key={opt.value} type="button" onClick={() => onChange(opt.value)}
            className={`px-2.5 py-1 text-xs font-medium transition-colors ${lang === opt.value ? 'bg-[#0669F7] text-white' : 'bg-white text-[#7A7B7A] hover:bg-[#F5F7FA] hover:text-[#25282A]'}`}>
            {opt.label}
          </button>
        ))}
      </div>
...
export function ContractDocument({ contract, documentRef, lang = 'ko' }: Props) {
  const s = CONTRACT_STRINGS[lang]
  ...
  return (
    <div ref={outerRef} style={{ width: '100%', overflow: 'hidden', display: 'flex', justifyContent: 'center' }}>
      <div ref={scaleRef} style={{ width: `${DOC_WIDTH}px` }}>
        <div ref={documentRef} style={{ width: `${DOC_WIDTH}px`, backgroundColor: '#ffffff', fontFamily: "'Malgun Gothic', '맑은 고딕', 'Apple SD Gothic Neo', sans-serif", color: '#222222', padding: '40px 48px', boxSizing: 'border-box' }}>

          {/* Header */}
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            <div style={{ marginBottom: '12px' }}>
              <span style={{ fontSize: '22px', fontWeight: 900, color: '#0669F7', letterSpacing: '-0.5px' }}>GADA</span>
              <span style={{ fontSize: '11px', marginLeft: '6px', color: '#666', fontWeight: 400 }}>{s.brand_sub}</span>
            </div>
            <div style={{ display: 'inline-block', borderTop: '2px solid #1a4fa0', borderBottom: '2px solid #1a4fa0', padding: '8px 32px', fontSize: '18px', fontWeight: 800, letterSpacing: lang === 'ko' ? '4px' : '1px', color: '#1a4fa0' }}>
              {s.title}
            </div>
          ...
          {/* Intro */}
          <p style={{ fontSize: '12px', lineHeight: '1.8', color: '#444', marginBottom: '20px', textAlign: 'justify' }}>{s.intro}</p>

          {/* Detail table */}
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #CCCCCC', marginBottom: '20px', fontSize: '12px' }}>
            <tbody>
              ...
              <Row label={s.label_wage} value={<span style={{ fontWeight: 700, color: '#0669F7', fontSize: '13px' }}>{fmtVND(contract.dailyWage)}</span>} />
          ...
          {/* Terms */}
          <div style={{ border: '1px solid #CCCCCC', borderRadius: '3px', padding: '14px 16px', backgroundColor: '#FAFAFA', marginBottom: '24px' }}>
            <p style={{ fontSize: '11px', fontWeight: 700, color: '#1a4fa0', marginBottom: '8px' }}>{s.terms_title}</p>
          ...
          {/* Signatures */}
          <div style={{ border: '1px solid #CCCCCC', borderRadius: '3px', padding: '16px', marginBottom: '20px' }}>
            <p style={{ fontSize: '11px', fontWeight: 700, color: '#1a4fa0', marginBottom: '16px' }}>{s.sig_title}</p>
            <div style={{ display: 'flex', gap: '20px' }}>
              {/* Worker */}
              <div style={{ flex: 1 }}>
                <div style={{ border: '1px solid #CCCCCC', borderRadius: '3px', backgroundColor: '#FAFAFA', padding: '12px', minHeight: '130px', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#444', marginBottom: '4px' }}>{s.party_a}</div>
                  <div style={{ fontSize: '11px', color: '#666', marginBottom: '8px' }}>{s.name_prefix}: {contract.workerName ?? '_________________'}</div>
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderTop: '1px dashed #CCCCCC', paddingTop: '8px', minHeight: '70px' }}>
                    {contract.workerSigUrl ? (
                      <img src={contract.workerSigUrl} alt={s.party_a} style={{ maxHeight: '64px', maxWidth: '100%', objectFit: 'contain' }} crossOrigin="anonymous" />
                      ...
                      <span style={{ fontSize: '11px', color: '#BBB', fontStyle: 'italic' }}>{s.sig_pending}</span>
                    )}
                  </div>
                  {contract.workerSignedAt && (
                    <div style={{ fontSize: '10px', color: '#888', textAlign: 'center', marginTop: '4px' }}>{s.sig_date}: {formatDate(contract.workerSignedAt, lang)}</div>
                  )}
                </div>
              </div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ border: '1px solid #CCCCCC', borderRadius: '3px', backgroundColor: '#FAFAFA', padding: '12px', minHeight: '130px', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#444', marginBottom: '4px' }}>{s.party_b}</div>
                  <div style={{ fontSize: '11px', color: '#666', marginBottom: '8px' }}>
                    {contract.companyName ? `${s.company_prefix}: ${contract.companyName}` : `${s.manager_prefix}: ${contract.managerName ?? '_________________'}`}
                  </div>
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderTop: '1px dashed #CCCCCC', paddingTop: '8px', minHeight: '70px' }}>
                    {(contract.companySigUrl ?? contract.managerSigUrl) ? (
                      <img src={(contract.companySigUrl ?? contract.managerSigUrl)!} alt={s.party_b} style={{ maxHeight: '64px', maxWidth: '100%', objectFit: 'contain' }} crossOrigin="anonymous" />
                    ) : null}
                  </div>
                  {contract.managerSignedAt && (contract.companySigUrl ?? contract.managerSigUrl) && (
                    <div style={{ fontSize: '10px', color: '#888', textAlign: 'center', marginTop: '4px' }}>{s.stamp_date}: {formatDate(contract.managerSignedAt, lang)}</div>
                  )}
                </div>
              </div>
            </div>
...
export function ContractDownloadButton({ documentRef, contractId, lang = 'ko' }: DownloadButtonProps) {
  const s = CONTRACT_STRINGS[lang]
  ...
  return (
    <button type="button" onClick={handleDownload} disabled={isGenerating}
      className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#0669F7] text-white font-medium text-sm hover:bg-[#0557D4] disabled:opacity-40 transition-colors">
      {isGenerating ? (
        <>
